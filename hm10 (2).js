class NewComicSource extends ComicSource {
    ...
    version = "2.1.0"
    minAppVersion = "1.0.0"
    description = '韩漫很全（搜索功能优化）+会员备用图源 + 已绕过SSL证书'
    url = "https://ssvip.mwtt.cc"

    // ✅ 【核心修复1】全局开启SSL证书绕过（Venera标准写法）
    allowUntrustedCertificate = true

    // ✅ 更新后的可用域名列表
    baseDomain = "https://www.manwaai.cc"
    backupDomains = [
        "https://www.mwxi.cc",
        "https://www.manwadu.cc",
        "https://www.manwami.cc",
        "https://www.manwala.cc"
    ]

    // 自动检测可用域名（timeout 已改为 3000ms，✅ 【核心修复2】加了证书绕过参数）
    async getAvailableDomain() {
        const domains = [this.baseDomain, ...this.backupDomains];
        for (const domain of domains) {
            try {
                const res = await Network.get(domain, { 
                    timeout: 3000, 
                    allowUntrustedCertificate: true 
                });
    ...
    // 图片请求重试（maxRetries 已改为 2，✅ 【核心修复3】所有请求加证书绕过）
    ...
    init() {
        console.log("韩漫（搜索版）已初始化，SSL证书绕过已开启")
    }
