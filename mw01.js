  ...
  comic = {
    loadInfo: async (id) => {
      try {
        ...
        const chapterUrl = `${this.api}/comic/chapter`;
        const chapterParams = {
          comicId: data.id,
          page: 1,
          pageSize: 1000,
    ...
    loadEp: async (comicId, epId) => {
      try {
        ...
        const params = {
          page: 1,
          pageSize: 1000,
          imageSource: "https://fm.mwtt.cc",
        };

        const response = await this.fetchJson(url, { params });
        const imageList = response.data?.images || [];

        if (imageList.length === 0) throw new Error("章节图片列表为空");

        const images = imageList.map((img) => String(img.url || "").trim()).filter(Boolean);
        if (images.length === 0) throw new Error("未找到有效的图片地址");
        ...
        for (const source of fallbackSources) {
          try {
            const retryParams = { page: 1, pageSize: 1000, imageSource: source };
            const retryResponse = await this.fetchJson(url, { params: retryParams });
            const retryImages = retryResponse.data?.images || [];
            if (retryImages.length > 0) {
              return { images: retryImages.map(img => String(img.url || "").trim()).filter(Boolean) };
            }
          } catch (e) {
            continue;
          }
        }
        throw new Error(`所有图源均失败: ${error.message}`);
      }
    },
  };
}