...
class ManWaBa extends ComicSource {
  ...
  // === 关键修改：更新API地址 ===
  api = "https://www.manwaku.com/api";  // 从 mhtmh.org 改为 manwaku.com
  ...
  init() {
    ...
    this.fetchJson = async (
      ...
      let res = await Network.sendRequest(method, url, defaultHeaders, payload);
      if (res.status !== 200) {
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      let data = await this.fetchJson(url, {}).then(
        (res) => res.data
      );
    ...
    loadEp: async (comicId, epId) => {
      ...
      // 更新图源地址，使用有效的图源
      let imageSources = [
        "https://tu.mihoutao.vip",      // 稳定图源
        "https://tu.mhttu.cc",           // 备用图源1
        "https://by.mihoutao.vip"        // 备用图源2
      ];
      
      let params = {
        page: 1,
        pageSize: 1,
        imageSource: imageSources[0],  // 使用第一个图源
      };
  ...
  settings = {
    image_source: {
      ...
      type: "select",
      options: [
        { value: "https://tu.mihoutao.vip", text: "稳定图源" },
        { value: "https://tu.mhttu.cc", text: "高速图源" },
        { value: "https://by.mihoutao.vip", text: "备用图源" }
      ],
      default: "https://tu.mihoutao.vip"
    }
  };
}
