class NewComicSource extends ComicSource {
    ...
    account = {
        login: async (account, pwd) => {
            let res = await Network.post("https://manwana.cc/api/user/userarr/login", {
        ...
        registerWebsite: "https://manwana.cc/user/register/"
    }
    ...
    explore = [
        {
            ...
            load: async () => {
                let url = "https://manwana.cc/cate/19plus?page=1"
                let res = await Network.get(url, {
                    "Referer": "https://manwana.cc/cate/",
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                })
                if (res.status !== 200) {
                    throw "Invalid status code: " + res.status
    ...
    categoryComics = {
        load: async (category, param, options, page) => {
            let sitePage = page + 1;
            let url = `https://manwana.cc/cate/19plus?page=${sitePage}`;

            let res = await Network.get(url, {
                "Referer": "https://manwana.cc/",
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            })
    ...
    search = {
        load: async (keyword, options, page) => {
            let res = await Network.get(`https://manwana.cc/api/search?keyword=${encodeURIComponent(keyword)}&type=mh&page=${page}&pageSize=20`, {
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            })
            if (res.status !== 200) {
                throw "Invalid status code: " + res.status
    ...
    favorites = {
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            ...
            if (isAdding) {
                ...
                let res = await Network.post("https://manwana.cc/api/user/bookcase/add", {
                ...
                let res = await Network.post("https://manwana.cc/api/user/bookcase/del", {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                }, `articleid=${id}`)
        ...
        loadComics: async (page, folder) => {
            let res = await Network.post("https://manwana.cc/api/user/bookcase/ajax", {
                "Content-Type": "application/x-www-form-urlencoded",
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            }, `page=${page}`)
            if (res.status !== 200) throw "Invalid status code: " + res.status
            ...
            let comics = json["data"].map(e => ({
                ...
                subTitle: e["author"],
                cover: e["cover"],
                id: "https://manwana.cc" + e["info_url"]
            }))
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            let res = await Network.get(`https://manwana.cc${id}`, {
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            })
            if (res.status !== 200) throw "Invalid status code: " + res.status
            let document = new HtmlDocument(res.body)
            let infoRes = await Network.get(`https://manwana.cc/api${id}`, {
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
            })
        ...
        loadEp: async (comicId, epId) => {
            ...
            let res = await Network.get(
                `https://manwana.cc/api/comic/image/${id}?page=1&page_size=${picCount}&image_source=${imageSource}`,
                {
                    "Referer": `https://manwana.cc${ep}`,
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
                }
            )
            if (res.status !== 200) throw "Invalid status code: " + res.status
        ...
        matchBriefIdRegex: "https://manwana.cc/(\\d+)/"
    }
