...
class ManWaAi extends ComicSource {
  ...
  async getAvailableDomain() {
    const domains = [...this.backupDomains];
  ...
  async getBestImageSource() {
    ...
    if (this._cachedImageSource && Date.now() - this._cacheTime < this.CACHE_DURATION) {
      console.log("使用缓存的图源:", this._cachedImageSource);
    ...
    const tests = this.imageSources.map(async (source, index) => {
      try {
        ...
        // 用一个小请求测试速度
        const testUrl = source + "/favicon.ico";
        await Network.sendRequest("HEAD", testUrl, {}, null, { timeout: 3000 });
        const responseTime = Date.now() - startTime;
        console.log(`图源 ${index + 1} (${source}) 响应时间: ${responseTime}ms`);
        return { source, index, responseTime };
      } catch (e) {
        console.log(`图源 ${index + 1} (${source}) 不可用`);
        return null;
      }
    });

    const results = await Promise.all(tests);
    const availableSources = results.filter(r => r !== null);
    
    if (availableSources.length === 0) {
      console.warn("所有图源都不可用，使用第一个");
      this._cachedImageSource = this.imageSources[0];
    } else {
      // 选择响应最快的
      availableSources.sort((a, b) => a.responseTime - b.responseTime);
      this._cachedImageSource = availableSources[0].source;
      console.log(`✅ 选择最快图源: ${this._cachedImageSource} (${availableSources[0].responseTime}ms)`);
    }
  ...
  comic = {
    ...
    loadEp: async (comicId, epId) => {
      ...
      try {
        ...
        console.error(`使用图源 ${bestImageSource} 失败:`, e);
      }
