class NewComicSource extends ComicSource {
    ...
    version = "2.2.0"
    minAppVersion = "1.0.0"
    description = '韩漫很全（搜索功能优化）+ 流量/WiFi通用（已忽略证书）'
    ...
    async retryImageRequest(url, options, maxRetries = 2) {
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                let opt = {
                    ...options,
                    skipSslValidation: true  // ✅ 核心：忽略证书
                };
                let res = await Network.get(url, opt)
                if (res.status === 200) {
                    return res
                }
    ...
    init() {
        console.log("韩漫（搜索版）已初始化（已开启忽略SSL证书）")
    }
